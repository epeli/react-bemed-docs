...
-   Not limited to just styles: Define custom classes and other props in the
    "BEMEd" component definitions
...
We'll introduce the libary via short tutorial which does not expect any pior
knowledge of Styled Components or BEM.
...
file but we can also use the `css` CSS-in-JS tool and write the styles with
...
automatically from the parent component (in this case `MyButton__Icon`) and
are exposed as properties of the parent:
...
automatically under the `.MyButton__Icon` selector so it won't leak
